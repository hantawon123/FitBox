...
class AdminNotificationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        binding.tvNotificationScreenTitle.text = "관리자 알림"
    ...
    private fun loadNotifications() {
        val adminUserId = SessionManager(requireContext()).getUser()?.id ?: return
        viewLifecycleOwner.lifecycleScope.launch {
            val notifications =
                repository.getNotifications(adminUserId).getOrDefault(emptyList())
            adapter.submitList(notifications)
            repository.markAllAsRead(adminUserId)
